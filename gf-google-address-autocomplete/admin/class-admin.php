    ...
    function pc_gf_api_key_set_fields() {
		...
		<div class="wrap">
			...
			<?php
				...
				if(isset($_POST['pcafe_load_with_async'])) {
					update_option('pcafe_load_with_async', sanitize_text_field($_POST['pcafe_load_with_async']));
				}

				$pc_gf_google_api_key	=	get_option('pc_gf_google_api_key');
				$is_async_checked  =	get_option('pcafe_load_with_async');

				$select_yes = $is_async_checked ? 'selected' : '';
				$select_no = ! $is_async_checked ? 'selected' : '';
				...
				<form method="post">
					<table class="form-table">
						<tbody>		
							...
							<tr>
								<th><label><?php _e("Load with Async", "gravityforms"); ?></label></th>
								<td>
									<select name="pcafe_load_with_async" id="pcafe_load_with_async">
										<option <?php echo $select_yes; ?> value="1"><?php _e("Yes", "gravityforms"); ?></option>
										<option <?php echo $select_no; ?> value="0"><?php _e("No", "gravityforms"); ?></option>
									</select>
								</td>
							</tr>
							<tr valign="top">
